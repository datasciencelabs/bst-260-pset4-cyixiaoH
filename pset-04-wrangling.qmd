...
population <- population |>
  ...
  # remove POP_ from year and pars year and population to numeric
  mutate(year = as.integer(str_remove(year, "POP_")),
...
population |> 
  ...
  ggplot(aes(x = population, y = reorder(state_name, -sum_pop))) +
  geom_col() +
  facet_wrap(~year, ncol = 2) +
  labs(title = "Population by State for 2020 and 2021",
       x = "State",
       y = "Population") +
...
regions <- map_df(seq_len(nrow(regions)), 
                  ~tibble(state_name = regions$states[[.x]],
                          region = as.integer(regions$region[[.x]]),
  ...
  mutate(region_name = if_else(
    ...
    "NY/NJ/PR/VI", 
    region_name)) |>
  # make sure we only have the 52 states
...
Response: 
The request was successful with HTTP 200, but the dataset returned only 1000 rows because of the default limit of API. 
```{r}
...
cases <- resp_body_json(response, simplifyVector = TRUE) |>
  as_tibble() |>
  transmute(state = state,
            date = as_date(end_date),
            cases = as.numeric(new_cases)) |>
  drop_na(state, date, cases)
```
...
cases |>
  ...
  drop_na(population, region_name) |>
  mutate(cases_100k = (cases / population) * 1e5) |>
  ggplot(aes(x = date, y = cases_100k, group = state, color = state)) +
  geom_line(linewidth = 0.3) +
  facet_wrap(~region_name, ncol = 2) +
  labs(title = "New COVID-19 Cases per 100k Population in 2020-2021",
       x = "Date",
       y = "Cases per 100k") +
  theme_minimal(base_size = 8) +
...
cases |> 
  ...
  kable(caption = "Total COVID-19 Cases by Month (2020–2021)")
```
...
deaths |>
  ...
  geom_col() +
  labs(title = "COVID-19 Deaths (TOP 10 States)",
       x = "State",
       y = "Deaths") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
